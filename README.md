...
The appliance has a Twitter bot built into it which will allows it to sell instances on a Twitter account if configured to do so.  This functionality was built to allow the sale of instances without requiring them to be sold on the pool operator's site.
